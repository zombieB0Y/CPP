...
ClapTrap::ClapTrap() : name("none"), HP(10), ENG(10), ATT(0), max_hp(10)
{
...
ClapTrap::ClapTrap(std::string _name) : name(_name), HP(10), ENG(10), ATT(0), max_hp(10)
{
...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (this->HP > 0 && this->ENG > 0)
	{
		...
		if (this->HP + amount <= this->max_hp) {
			...
			std::cout << "ClapTrap " << name << " is healing, causing " << this->max_hp - this->HP << " points of heal!" << std::endl;
			this->HP = max_hp;
		}
	}
	else if (this->HP == 0)
...
unsigned int	ClapTrap::Getmax_hp(void) const {
	return max_hp;
}